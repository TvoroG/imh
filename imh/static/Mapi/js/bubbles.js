...
function Point(x, y, z, size, color) {
    ...
    this.draw = function (bubbleShape, dx, dy) {
        ...
        if (bubbleShape == "square") {
            ctx.beginPath();
            ctx.fillRect(this.curPos.x/2 + dx, this.curPos.y/2 + dy, this.radius * 1.5, this.radius * 1.5);
        } else {
            ctx.beginPath();
            ctx.arc(this.curPos.x/2 + dx, this.curPos.y/2 + dy, this.radius/2, 0, Math.PI * 2, true);
            ctx.fill();
        }
    };
}
